...
unique(convictrelevantdata$County.Colony)
...
sum(is.na(convictrelevantdata$Convict.Name))
sum(is.na(convictrelevantdata$Sentence.details))
sum(is.na(convictrelevantdata$Date.of.Departure))
sum(is.na(convictrelevantdata$Place.of.Arrival))
sum(is.na(convictrelevantdata$Place.of.Sentence))
sum(is.na(convictrelevantdata$Sentence.Duration))
sum(is.na(convictrelevantdata$Date.of.Sentence))
sum(is.na(convictrelevantdata$Ticket.of.Leave))
...
predictionconvictsdata$County.Colony <- as.numeric(predictionconvictsdata$County.Colony)
...
#include only complete cases
predictionconvictsdata <- predictionconvictsdata[complete.cases(predictionconvictsdata),]
...
#accuarcy for performance measures

rm(Targets_train,Targets_value,trainset,model,predictTestSet)
...
#ctree
convicts.ctree <- ctree(Sentence.Duration ~ County.Colony + Place.of.Arrival + Date.of.Departure + Ticket.of.Leave,
                        predictionconvictsdata, controls = ctree_control(maxdepth = 3))
print(convicts.ctree)
plot(convicts.ctree)

convicts.rpart.predict <- predict(convicts.rpart,predictionconvictsdata,type= "vector")

table(convicts.rpart.predict,predictionconvictsdata$Sentence.Duration)

rm(convicts.ctree,convicts.rpart,convicts.rpart.predict)
...
train_data <- predictionconvictsdata[,c(2,4,5,7)] #relevant attributes
...
if (packageVersion("kohonen") < 3){
  system.time(som_model <- supersom(train_data_matrix, 
                                    grid=som_grid, 
                                    rlen=1000, 
                                    alpha=c(0.9,0.01),
                                    n.hood = "circular",
...
var_unscaled <- aggregate(as.numeric(train_data[,var]), by=list(som_model$unit.classif), FUN=mean, simplify=TRUE)[,2]
plot(som_model, type = "property", property=var_unscaled, main=names(train_data)[var], palette.name=coolBlueHotRed)


rm(var_unscaled, var,train_data,train_data_matrix,som_grid,som_model,coolBlueHotRed)



#Discuss whether or not the duration of sentences tended to change over time.


predictionconvictsdata$Date.of.Sentence <- gsub('8142', '1842', predictionconvictsdata$Date.of.Sentence)



for (i in 1:nrow(predictionconvictsdata))
{
  if(nchar(predictionconvictsdata[i,"Date.of.Sentence"]) == 2)
  {
    year <- i
    predictionconvictsdata[i,"Date.of.Sentence"]
  }
    
}
predictionconvictsdata[year,"Date.of.Sentence"] = "1828"

plot(predictionconvictsdata$Sentence.Duration,predictionconvictsdata$Date.of.Sentence,xlab = "duration",ylab = "year")
